...
func EmbeddedYAMLs(ctx context.Context, kubeClient kubernetes.Interface, namespace string,
	appliers []EmbeddedYAMLRefsApplier,
) (bool, error) {
	created := false
	...
	for _, applier := range appliers {
		for _, ref := range applier.Refs {
			...
			if err != nil {
				return created, errors.Wrapf(err, "failed to apply %s", ref.Name)
			}
